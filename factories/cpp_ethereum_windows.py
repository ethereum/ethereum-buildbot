    ...
    if isPullRequest is False:
        for step in [
            ...
            FileUpload(
                ...
                masterdest=Interpolate("public_html/builds/%(prop:buildername)s/Ethereum (++)-%(prop:version)s-win64-%(kw:time_string)s-%(kw:short_revision)s.exe"),
                url=Interpolate("/builds/%(prop:buildername)s/Ethereum (++)-%(prop:version)s-win64-%(kw:time_string)s-%(kw:short_revision)s.exe")
            )
        ]: factory.addStep(step)
